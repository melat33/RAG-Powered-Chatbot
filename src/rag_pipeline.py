...
class AdvancedFinancialRAG:
        ...
        class SimpleQueryEnhancer:
            def analyze_query(self, question: str):
                ...
                business_context = {
                    ...
                    "needs_trend_analysis": any(
                        word in question_lower
                        for word in ["trend", "pattern", "over time"]
                    ),
                    "is_comparative": any(
                        word in question_lower
                        for word in ["compare", "vs", "versus", "difference"]
                    ),
                    "needs_root_cause": any(
                        word in question_lower for word in ["why", "reason", "cause"]
                    ),
                    "volume_analysis": any(
                        word in question_lower for word in ["many", "often", "frequent"]
                    ),
                }
            ...
            def enhance_query(self, question: str, analysis: dict):
                ...
                if analysis.get("products"):
                    product_context = (
                        f"financial complaint about {analysis['products'][0]}"
                    )
    ...
    def _extract_metadata_field(
        self, meta: Dict, field_names: List[str], default: str = "Unknown"
    ) -> str:
    ...
    def _get_product_from_metadata(self, meta: Dict) -> str:
        ...
        product = self._extract_metadata_field(
            meta,
            [
                "product_category",
                "product",
    ...
    def _get_issue_from_metadata(self, meta: Dict) -> str:
        ...
        return self._extract_metadata_field(
            meta,
    ...
    def calculate_confidence_score(self, retrieved_data: Dict) -> Dict:
        ...
        # Total score
        total_score = (
            semantic_score + retrieval_score + diversity_score + metadata_score
        )
    ...
    def generate_business_insights(
        self, question: str, retrieved_data: Dict, confidence: Dict
    ) -> Dict:
        """
        ...
        for meta in retrieved_data["metadata"]:
            ...
            if any(
                term in issue_lower
                for term in ["fraud", "unauthorized", "theft", "scam", "identity"]
            ):
                severities["high"] += 1
            elif any(
                term in issue_lower
                for term in ["fee", "charge", "billing", "payment", "interest"]
            ):
                severities["medium"] += 1
            else:
                severities["low"] += 1
        ...
        if analysis["business_context"]["is_comparative"]:
            ...
            insights = self._generate_general_insights(
                products, issues, severities, question
            )
        ...
        insights["confidence_context"] = {
            ...
            if confidence["retrieved_count"] >= 3
            else "Moderate"
            if confidence["retrieved_count"] >= 1
            else "Weak",
        }
    ...
    def _generate_general_insights(
        self, products: Dict, issues: Dict, severities: Dict, question: str
    ) -> Dict:
        ...
        top_issues = sorted(filtered_issues.items(), key=lambda x: x[1], reverse=True)[
            :3
        ]

        total_complaints = (
            sum(filtered_products.values())
            if filtered_products
            else sum(products.values())
        )

        return {
            "executive_summary": f"Analysis of {total_complaints} relevant complaints reveals key customer pain points.",
            "key_findings": [
                f"Top product category: {top_products[0][0]} ({top_products[0][1]} complaints)"
                if top_products
                else "No product data available",
                f"Most frequent issue: {top_issues[0][0]} ({top_issues[0][1]} occurrences)"
                if top_issues
                else "No issue data available",
                f"Severity distribution: {severities['high']} high, {severities['medium']} medium, {severities['low']} low priority",
            ],
            "patterns_detected": [
                f"Product concentration: {len(filtered_products)} different products mentioned"
                if filtered_products
                else "Limited product data",
                f"Issue diversity: {len(filtered_issues)} distinct issues identified"
                if filtered_issues
                else "Limited issue data",
            ],
            "recommendations": [
    ...
    def _generate_comparative_insights(
        self, products: Dict, issues: Dict, question: str
    ) -> Dict:
        ...
        if len(filtered_products) < 2:
            return {
                ...
                "key_findings": [
                    "Need complaints from at least 2 different products for comparison"
                ],
                "patterns_detected": [],
                "recommendations": [
                    "Ask more general questions to gather broader data"
                ],
        ...
        return {
            ...
            "key_findings": [
                ...
                if sorted_products[-1][1] > 0
                else "Insufficient data for ratio",
            ],
            "patterns_detected": [
    ...
    def _generate_trend_insights(
        ...
        total_issues = (
            sum(filtered_issues.values()) if filtered_issues else sum(issues.values())
        )

        return {
            "executive_summary": f"Trend analysis based on {sum(products.values())} relevant complaints.",
            "key_findings": [
                f"Top trending issue: {top_issues[0][0]} ({top_issues[0][1]} occurrences)"
                if top_issues
                else "No trend data available",
                f"Issue frequency spread: {len(filtered_issues)} distinct issues identified"
                if filtered_issues
                else "Limited issue data",
                f"Product coverage: Complaints from {len([p for p in products.keys() if p != 'Unknown'])} product categories",
            ],
            "patterns_detected": [
                f"Dominant issues: Top 3 issues account for {sum(c for _, c in top_issues[:3])/total_issues*100:.1f}% of complaints"
                if len(top_issues) >= 3 and total_issues > 0
                else "Insufficient data for pattern detection"
            ],
            "recommendations": [
    ...
    def _generate_root_cause_insights(
        self, products: Dict, issues: Dict, question: str
    ) -> Dict:
        ...
        top_categories = sorted(
            categorized_issues.items(), key=lambda x: x[1], reverse=True
        )
        ...
        return {
            ...
            "key_findings": [
                f"Primary root cause category: {top_categories[0][0]} ({top_categories[0][1]} issues)"
                if top_categories[0][1] > 0
                else "No clear root cause pattern",
                f"Secondary category: {top_categories[1][0]} ({top_categories[1][1]} issues)"
                if len(top_categories) > 1 and top_categories[1][1] > 0
                else "",
                f"Root cause distribution: {', '.join([f'{c}: {n}' for c, n in top_categories if n > 0])}",
            ],
            "patterns_detected": [
                f"Systemic issues: {top_categories[0][0]} appears most frequently"
                if top_categories[0][1] > 0
                else "",
                f"Multiple root cause categories identified: {len([c for c in categorized_issues.values() if c > 0])}",
            ],
            "recommendations": [
                f"Address systemic {top_categories[0][0].lower()} issues first"
                if top_categories[0][1] > 0
                else "Collect more specific complaint data",
    ...
    def ask(self, question: str, product_filter: Optional[str] = None) -> Dict:
        ...
        retrieved_data = self.retrieve_complaints(
        ...
        for i, (meta, distance) in enumerate(
            zip(retrieved_data["metadata"], retrieved_data.get("distances", [])), 1
        ):
            similarity = (1 - distance) * 100 if distance is not None else 0

            product = self._get_product_from_metadata(meta)
            issue = self._get_issue_from_metadata(meta)
            company = self._extract_metadata_field(
                meta, ["company", "Company", "bank", "Bank"], "Unknown"
            )
            state = self._extract_metadata_field(
                meta, ["state", "State", "location", "Location"], "Unknown"
            )
            date_received = self._extract_metadata_field(
                meta, ["date_received", "date", "Date", "received_date"], "Unknown"
            )
        ...
        total_retrieved = (
            self.analytics["performance_stats"].get("total_retrieved", 0)
            + retrieved_data["count"]
        )
        ...
        response = {
            ...
            "retrieval_stats": {
                ...
                "products_covered": len(
                    set(s["product"] for s in sources if s["product"] != "Unknown")
                ),
                "issues_identified": len(
                    set(s["issue"] for s in sources if s["issue"] != "General")
                ),
    ...
    def get_performance_report(self) -> Dict:
        ...
        return {
            "summary": {
                ...
                "avg_complaints_per_query": round(
                    stats.get("avg_retrieval_count", 0), 1
                ),
                "system_uptime": "Active",
            },
            "recent_queries": self.analytics["query_log"][-5:]
            if self.analytics["query_log"]
            else [],
            "recommendations": [
                "System performing well for business queries"
                if stats["success_rate"] > 70
                else "Consider improving query understanding",
                "Good complaint retrieval coverage"
                if stats.get("avg_retrieval_count", 0) >= 3
                else "May need more diverse complaint data",
                "Ready for production use"
                if stats["total_queries"] >= 10 and stats["success_rate"] > 60
                else "Needs more testing",
            ],
        }
