...
def create_product_dashboard(
    df: pd.DataFrame, viable_df: pd.DataFrame, save_path: Path = None
):
    ...
    if "Product_Category" in viable_df.columns:
        our_counts = viable_df[viable_df["Product_Category"].isin(cfg.TARGET_PRODUCTS)][
            "Product_Category"
        ].value_counts()
        fig.add_trace(
            go.Pie(
    ...
    if "Product_Category" in df.columns:
        product_counts = df[df["Product_Category"].isin(cfg.TARGET_PRODUCTS)][
            "Product_Category"
        ].value_counts()
        fig.add_trace(
            go.Bar(
                x=product_counts.index, y=product_counts.values, marker_color="#45B7D1"
            ),
            row=2,
            col=2,
        )
    ...
    fig.update_layout(
        height=800, showlegend=False, title_text="Product Analysis Dashboard"
    )
...
def create_text_length_plot(df: pd.DataFrame, save_path: Path = None):
    ...
    fig.add_trace(
        go.Histogram(
            x=df["Text_Length_Words"], nbinsx=50, marker_color="#4ECDC4", name="Words"
        ),
        row=1,
        col=1,
    )
