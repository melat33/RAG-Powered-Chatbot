...
def create_evaluation_report(results_df):
    ...
    if "confidence" in results_df.columns:
        ...
        axes[0, 0].axvline(
            40, color="orange", linestyle="--", alpha=0.7, label="Medium (40-70)"
        )
        axes[0, 0].legend()
    ...
    if "keyword_score" in results_df.columns:
        bars = axes[0, 1].bar(
            range(len(results_df)), results_df["keyword_score"].values, color="#4ECDC4"
        )
    ...
    # Calculate metrics
    avg_conf = (
        results_df["confidence"].mean() if "confidence" in results_df.columns else 0
    )
    avg_keyword = (
    ...
    success_rate = (
        results_df["success"].mean() * 100 if "success" in results_df.columns else 0
    )
...
def print_evaluation_table(results_df):
    ...
    for i, row in results_df.iterrows():
        ...
        print(
            f"   Confidence: {row.get('confidence', 0):.1f}/100 | "
