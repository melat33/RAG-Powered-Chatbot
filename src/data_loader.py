...
def load_complaints_data(
    filepath: str, sample_size: Optional[int] = None
) -> pd.DataFrame:
    """
...
def filter_complaints_streaming(
    filepath: str, product_mapping: dict, target_products: list
) -> pd.DataFrame:
    """
    ...
    for i, chunk in enumerate(
        ...
        chunk["Product_Category"] = (
            chunk["Product"].map(product_mapping).fillna("Other")
        )
        ...
        if (i + 1) % 10 == 0:
            logger.info(
                f"   📊 Chunk {i+1}: Processed {total_processed:,} → Kept {total_filtered:,} ({total_filtered/total_processed*100:.1f}%)"
            )
...
def load_with_optimized_types(
    filepath: str, usecols: Optional[list] = None
) -> pd.DataFrame:
    """
    ...
    if usecols:
        dtype_subset = {
            col: DTYPE_STRATEGY[col] for col in usecols if col in DTYPE_STRATEGY
        }
    else:
