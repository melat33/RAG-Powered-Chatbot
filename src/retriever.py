...
class HybridRetriever:
    ...
    def semantic_retrieve(
        self, query: str, k: int = 3, filter_product: Optional[str] = None
    ) -> Dict:
    ...
    def hybrid_retrieve(
        self, question: str, k: int = RETRIEVAL_K, filter_product: Optional[str] = None
    ) -> Dict:
        ...
        for enhanced_query in enhanced_queries[:2]:  # Use top 2 enhanced queries
            result = self.semantic_retrieve(
                enhanced_query, k=k // 2, filter_product=filter_product
            )

            # Add method tag
            for chunk, meta, dist in zip(
                result["chunks"], result["metadata"], result["distances"]
            ):
                semantic_results.append(
                    (chunk, meta, dist, f"semantic_{enhanced_query[:20]}...")
                )
