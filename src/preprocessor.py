...
def fast_filter_pipeline(df):
    ...
    print(
        f"📊 Filtered to {len(df_filtered):,} business-relevant complaints with narratives"
    )
    return df_filtered
...
def prepare_final_dataset(df):
    ...
    df["Cleaned_Narrative"] = (
        df["Consumer complaint narrative"].fillna("").astype(str).str.lower()
    )
    df["Cleaned_Narrative"] = df["Cleaned_Narrative"].str.replace(
        r"[^\w\s]", " ", regex=True
    )
    df["Cleaned_Narrative"] = (
        df["Cleaned_Narrative"].str.replace(r"\s+", " ", regex=True).str.strip()
    )
