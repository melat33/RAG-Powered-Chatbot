...
def plot_text_distribution(chunks_df, chunk_size, save=True):
    ...
    plt.axvline(
        chunk_size, color="red", linestyle="--", label=f"Chunk Size ({chunk_size})"
    )
    plt.axvline(
        np.median(lengths),
        color="green",
    ...
    if save:
        plt.savefig(
            REPORTS_DIR / "chunk_distribution.png", dpi=150, bbox_inches="tight"
        )
    plt.show()
...
def plot_chunks_per_complaint(chunks_df, save=True):
    ...
    plt.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Mean: {chunks_per.mean():.2f}",
    )
    ...
    if save:
        plt.savefig(
            REPORTS_DIR / "chunks_per_complaint.png", dpi=150, bbox_inches="tight"
        )
    plt.show()
